...
def cluster_pages(pages, page_text, n_keywords=12) -> dict:
    ...
    for seg, members in sorted(clusters.items(), key=lambda x: -len(x[1])):
        ...
        # cluster keywords = most common across members; top 2 used for fallback name
        ck = Counter()
        for m in members:
            ck.update(kw.get(m, []))
        out.append({
            "key": seg,
            "name": " ".join(w.title() for w in [w for w, _ in ck.most_common(2)]),
            "size": len(members),
            "pages": members,
            "hub_page": hub,
...
# --------------------------------------------------------------------------- #
def _anchor_from_page(url: str, meta: dict) -> str:
    """Derive a descriptive anchor text from page metadata.

    Priority: Title 1 -> H1-1 -> URL slug.
    Strips brand suffixes (text after | - – — separators), lowercases, caps at 6 words.
    """
    for field in ("Title 1", "H1-1"):
        ...
        for sep in (" | ", " - ", " – ", " — ", " :: "):
            if sep in raw:
                ...
                break
        anchor = " ".join(raw.lower().split()[:6])
        if anchor and anchor not in GENERIC_ANCHORS:
            return anchor
    # URL slug fallback
    path = urlparse(url).path.strip("/")
    slug = (path.split("/")[-1] if path else "").replace("-", " ").replace("_", " ")
    return " ".join(slug.lower().split()[:6]) or url
...
def link_candidates(graph, relate: dict, pages, max_per_page=5) -> list:
    ...
    link to. suggested_anchor is derived deterministically from the target page's
    Title 1 / H1-1 / URL slug; the linker-agent replaces these with richer anchors
    when it runs.
    """
    idx200 = [p for p in pages if is_html(p) and is_200(p) and indexable(p)]
    inl = {_norm(p["Address"]): _int(p.get("Unique Inlinks")) for p in idx200}
    orphans = {u for u, n in inl.items() if n == 0}
    under_linked = {u for u, n in inl.items() if 0 < n <= 1}
    meta = {_norm(p["Address"]): p for p in pages}
    ...
    for u in important:
        ...
        for e in relate.get(u, []):
            ...
            if e["score"] < 0.10:
                continue
            if any(seg in urlparse(v).path for seg in ("/author/", "/tag/", "/page/", "/category/")):
                continue
            cands.append({"target": v, "relatedness": e["score"], "shared_topics": e["shared"],
                          "suggested_anchor": _anchor_from_page(v, meta.get(v, {}))})
        cands.sort(key=lambda c: -(c["relatedness"] + (0.2 if c["target"] in orphans else 0.1 if c["target"] in under_linked else 0)))
        if cands:
            out.append({"source": u, "candidates": cands[:max_per_page]})
    return out
